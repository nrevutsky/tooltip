...
def save_to_event_db(pr_id, msg_id, msg_count, event_name):
    current_day = dt.datetime.now().day
    msg, created = DayEvent.objects.get_or_create(hour=dt.datetime.now().hour,
                                                  project_id=pr_id,
                                                  message_id=msg_id)
    msg_day, created_day = PeriodEvent.objects.get_or_create(day=dt.date.today(),
                                                             project_id=pr_id,
                                                             message_id=msg_id)
